...
cr_cards <- read_json(url, simplifyVector = TRUE)  #base way to read json files into r, first argument is file, simplifyVector = TRUE will put this into a data frame for us

# shortcut is 
cr_cards2 <- fromJSON(url) # this is a shortcut to load in json files

identical(cr_cards, cr_cards2) # this function will tell you if these two files are identical
```
...
```{r}
library(here)
donuts <- read_json(here("data","donuts2.json"), simplifyVector = TRUE) # when the data is imported this way, there is a table nested inside of the table that it spits out. This happens becuase the donuts2.json file is not in a consistent format and has a nested structure within it
donuts
class(donuts)
```
...
d2 <-
  donuts_tibble %>%
    ...
    #unnest_longer(topping) 
#%>% # could also expand topping portion
...
Load the data into a workable format:
```{r}
tri_results <- fromJSON(url)

# convert to tibble
ironman_tibble <- 
  tri_results  %>%  
    as_tibble() %>%
  readr::type_convert() #will run the parse guess to guess the column types and convert them into the correct column type to get the data in hopefully the right product
